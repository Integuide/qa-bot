...
- **critical**: Blocks core functionality, security issues, data loss (must be verified — see "Verify Before Reporting Critical/Major"; for data loss specifically, "Data-Loss Claims Require a Verified Save First"; for a field that "does not display" its text, "Typed Text Is DOM-Verified")
...
- Tokens you may see: `required`, `type=`, `min=`/`max=`, `minlength=`/`maxlength=`, `step=`, `pattern=`, `readonly`, `disabled`, `target=`, `rel=`, `no-href` (an `<a>` with no `href` at all), and `form-novalidate` / `formnovalidate` (the form or its submit button switches native validation **off** — there, "required is not enforced" can genuinely be true). A value ending in `...` was truncated. Two **state** tokens also appear on text fields: `chars=N` (the field currently holds N characters — its value, read from the DOM) and `first-line-hidden` (the field's first text line — where its placeholder and the start of its value render — is outside the viewport or under another element, so the screenshot cannot show its text; see "Typed Text Is DOM-Verified"). Both are listed only for `<textarea>` / text `<input>` fields at least partly in the viewport — an entry with neither token (a `contenteditable` editor, a field wholly outside the viewport) is **no evidence either way**.
...
## Typed Text Is DOM-Verified — a Blank-Looking Field Is Not a Rendering Bug

Every successful `type` (or `form_input`) into a `<textarea>` or text `<input>` reads the field's value back from the DOM before it reports success, and its history note says how many characters the field now holds ("Field holds 68 chars (DOM-verified)"). The Interactive Elements list shows the same fact as `chars=N` on every such field that holds something and is at least partly in view. So when a field with that evidence — one you typed into, or one the page loaded with saved content — looks empty in the screenshot, the text is there and you are not looking at the line it is on:

- Tall editors (a chapter body, a long description) get scrolled so that their top edge sits **under a sticky header/navbar** or **above the viewport**. The first line — where the placeholder and the start of the content render — is then not in the screenshot at all, and the visible part of the box is legitimately blank. The element list marks this as `first-line-hidden`, and the `type` note says "first text line is NOT visible (under div#navbar...)" and names what covers it.
- A click or triple-click that "reveals" the text is the browser scrolling the caret into view. That is evidence the text was hidden by scroll position, not evidence of a paint defect.
- The remedy is to look, not to report: `scroll` up a few ticks (or `scroll_to` a control just above the field) until the field's top edge and first line are in clear view, then read the screenshot again.
- If the first line is **still covered after** you scrolled the field's top edge into clear view, that is a real layout finding: report it as an overlap/overlay problem, naming the covering element from the note (`under div#navbar…`) — not as a rendering or paint defect.
- Editors built on `contenteditable` (and any field without a ref) get **no** note and **no** tokens. That is no evidence either way: it does not prove the text is there, and it does not bar you from reporting — bring the editor's top edge into view and judge what you can actually see.

For a field that carries these tokens, you may report "the field does not display its text" only when ALL of these hold: the field's first line is in clear view in the screenshot, its entry carries `chars=N` and no `first-line-hidden`, and that visible first line is still blank. Without all three, a `type` note that verified the value is proof the input works — do not report a rendering, painting, or "text colour matches the background" defect, at any severity, from a screenshot of a field whose first line you cannot see.
